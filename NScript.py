...
def pause_and_exit(message="Press Enter to close...", code=0):
    if INTERACTIVE_MODE:
        safe_input(message)
    else:
        print("[i] Non-interactive mode detected; skipping input prompt.")
    raise SystemExit(code)

def ask_continue(prompt, default=False):
    if not INTERACTIVE_MODE:
        print(f"[i] Non-interactive mode detected; using default answer: {'Yes' if default else 'No'}")
        return default
...
def check_unlock_status(session, cookie_value, device_id):
    try:
        ...
        if response_data.get("code") == 100004:
            ...
            pause_and_exit("Press Enter to close...", code=1)
        ...
        if is_pass == 1:
            ...
            pause_and_exit("Press Enter to close...", code=0)

        # Xiaomi payloads can vary by app/backend version; button_state=1 means request is allowed.
        if button_state == 1:
            print(col_g + f"[Account status]: " + Fore.RESET + f"It is possible to send the request (is_pass={is_pass}).")
            return True

        if is_pass == 4:
            if button_state == 2:
                print(col_g + f"[Account status]: " + Fore.RESET + f"Blocked to send requests until " f"{deadline_format} (Month/Day).")
                if ask_continue(f"Continue (" + col_b + f"Yes/No" +Fore.RESET + f")?: ", default=False):
                    return True
                else:
                    pause_and_exit("Press Enter to close...", code=0)
            elif button_state == 3:
            ...
            print(col_g + f"[Debug]: " + Fore.RESET + f"Unexpected status payload: {response_data}")
            pause_and_exit("Press Enter to close...", code=1)
    except Exception as e:
